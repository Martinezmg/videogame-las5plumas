...
public class Sight : MonoBehaviour
{
    ...
    {
        if (other.gameObject == target && isTargetInSight)
        {
    ...
    private void SightTarget()
    {
        ...
        foreach (Vector3 dir in Directions())
        {
            ...
            {
                if (hit.collider.gameObject == target && !isTargetInSight)
                {
    ...
    private void OnDrawGizmos()
    {
        ...
        foreach (Vector3 dir in Directions())
        {
            Gizmos.DrawRay(transform.position + sightCenter, dir * sightDistance);
        }      
    }
